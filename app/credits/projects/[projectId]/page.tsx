...
export async function generateMetadata({ params }: ProjectPageProps): Promise<Metadata> {
  try {
    ...
    const resolvedParams = await params
    return generateProjectSEO({ id: resolvedParams.projectId, title: 'Credit Project' })
  }
}
...
export default async function CreditProjectPage({ params }: ProjectPageProps) {
  const resolvedParams = await params
  ...
  const tokenData = {
    tokenId: `TOKEN-${project.id}`,
    contractAddress: '0x1234567890123456789012345678901234567890',
    tokenUri: `https://api.example.com/metadata/${project.id}`,
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {/* Dual Proof Panel */}
      <div className="mb-8">
        <h2 className="text-2xl font-bold mb-4">Credit & Token Proof</h2>
        <ProofPanel 
          creditData={{
            id: project.id,
            projectId: project.id,
            serialStart: 1000001,
            serialEnd: 1000000 + project.totals.issued,
            quantity: project.totals.issued,
      ...
      {project.vintages.length > 0 && (
        <div className="mb-8">
          <h2 className="text-2xl font-bold mb-4">Credit Vintages</h2>
